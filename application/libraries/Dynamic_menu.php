...
// para CodeIgniter Super Global Referencias o variables globales
  private $ci;
  private $ul_class           = 'id="sidebarnav" class="in"';
  private $li_parent          = '';
  private $li_nonparent       = '';
  private $link_class_np      = 'class="waves-effect waves-dark" aria-expanded="false"';
  private $link_class         = 'class="has-arrow waves-effect waves-dark" aria-expanded="false"';
  // --------------------------------------------------------------------
  /**
   * PHP5        Constructor
   *
          ...
          if ($show_menu == 'Y' && $parent_id == 0) {
            ...
            if ($is_parent == 'N') {
              $html_out .= '<li ' . $this->li_nonparent . '>
                                <a href=' . '"' . $url . '"' . $this->link_class_np . ' target=' . $target . '>
                                    <i class=' . $icon_class . '></i> <span class="hide-menu">' . $title . '</span>
                                </a> ';
            } else {
            ...
            $html_out.='<ul aria-expanded="false" class="collapse">';
  ...
  function get_childs($id)
  {
    ...
    foreach ($query->result() as $row) {
      ...
      if (in_array($id, $userRightsIds)) {
        if ($is_parent == 'Y') {
          $html_out .= '<li ' . $this->li_parent . '>
                                            <a ' . $this->link_class_np . ' target=' . $target . '>
